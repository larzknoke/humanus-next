...
type JQueryResult = {
  layerSlider?: (options: object) => void;
  hasClass?: (name: string) => boolean;
};
type JQueryLike = ((selector: string | Element) => JQueryResult) & {
  fn?: { layerSlider?: unknown };
};
declare global {
  interface Window {
    jQuery?: JQueryLike;
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      if (home && $.fn?.layerSlider) {
        ...
        if (!slider.hasClass?.("ls-container"))
          slider.layerSlider?.({
            responsive: false,
            ...
            skin: "v5",
            hoverPrevNext: false,
            skinsPath: "/assets/3rd/layerslider/skins/",
          });
      }
      const video = document.querySelector<HTMLVideoElement>("#player");
      document
        .querySelector("#play")
        ?.addEventListener("click", () => void video?.play());
      document
        .querySelector("#pause")
      ...
      document.querySelector("#duration")?.addEventListener("click", () => {
        ...
        if (output && video)
          output.textContent = `Die Dauer des Videos beträgt: ${video.duration} Sekunden.`;
      });
